...
class BST:
    ...
    def _are_adjacents_aux(self, parent, v1, v2):
        ...
        if not adjacents:
            left_adj = self._are_adjacents_aux(parent._left, v1, v2)
            right_adj = self._are_adjacents_aux(parent._right, v1, v2)
            adjacents = left_adj or right_adj
            
        if parent._left and parent._right:
            adjacents = (parent._left._value == v1 and parent._right._value == v2) or \
                (parent._left._value == v2 and parent._right._value == v1)
    ...
    def furthest(self):
        ...
        if self._root is None:
            ...
            values.append(self._root._value)
            self._furthest_aux(self._root, values)
        return values

    def _furthest_aux(self, node, values):
        if node is None:
            return

        if node._value > values[1]:
            values[1] = node._value
        elif node._value < values[0]:
            values[0] = node._value
    
        self._furthest_aux(node._left, values)
        self._furthest_aux(node._right, values)
