...
def decode_morse(bst, code):
    ...
    """
    Why is it necessary to have separate trees for encoding and 
    decoding even when the same combination of letters and codes 
    is being stored?
    
    Answer:
    Because the way we navigate the tree differs depending on 
    whether we are encoding or decoding, we need separate trees 
    for encoding and decoding. When we encode a letter, we 
    begin at the tree's root and work our way down the tree 
    until we find the leaf node that has the letter's code. 
    But, to decode a code, we must begin at the tree's base 
    and work our way down until we reach the leaf node that 
    contains the letter. This path depends on the order of 
    the dots and dashes in the code.
    """