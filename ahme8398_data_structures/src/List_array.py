...
class List:
    ...
    def _swap(self, i, j):
        ...
        if not self._is_valid_index(i):
            raise IndexError(f"Invalid index i: {i}")

        if not self._is_valid_index(j):
            raise IndexError(f"Invalid index j: {j}")
    ...
    def combine(self, source1, source2):
        ...
        while len(source1._values) > index or len(source2._values) > index:
            if len(source1._values) > index:
                value1 = source1._values.pop(0)
                self._values.append(value1)
            if len(source2._values) > index:
